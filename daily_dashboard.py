...
# K 線圖用：缺套件也不讓整支程式掛掉（K棒分頁會顯示提示）
try:
    import io

    import matplotlib

    matplotlib.use("Agg")  # 無視窗環境（GitHub Actions）必須用 Agg
...
# 「熱度榜」候選股池：常被新聞提到、流動性高的台股，可自行增減。
# 程式會在這池子裡找「同時有新聞熱度 + 技術動能也強」的個股。
HEAT_CANDIDATES = {
    "2330": "台積電", "2317": "鴻海", "2454": "聯發科", "2382": "廣達",
    "3231": "緯創", "6669": "緯穎", "2376": "技嘉", "2357": "華碩",
    "2308": "台達電", "3711": "日月光", "2303": "聯電", "3034": "聯詠",
    "2379": "瑞昱", "3443": "創意", "3661": "世芯", "3017": "奇鋐",
    "3324": "雙鴻", "3008": "大立光", "2412": "中華電", "2002": "中鋼",
    "2603": "長榮", "2609": "陽明", "2615": "萬海", "2882": "國泰金",
    "2881": "富邦金", "2891": "中信金", "2886": "兆豐金", "2409": "友達",
    "3481": "群創", "2327": "國巨", "2408": "南亞科", "2344": "華邦電",
    "2337": "旺宏", "6770": "力積電", "2474": "可成",
}
...
OHLC_CACHE = {}  # 共用快取：存每檔的完整 OHLCV，避免同一檔重複抓


def fetch_df(code):
    """抓完整開高低收+成交量（1 年），結果快取起來"""
    ...
    for suffix in (".TW", ".TWO"):
        try:
            ...
            pass
    OHLC_CACHE[code] = result
    time.sleep(0.3)  # 對 Yahoo 客氣一點，避免被限流
    return result
...
.tabs{display:flex;gap:6px;margin:10px 0 16px;position:sticky;top:0;background:#f6f5f1;padding:8px 0;z-index:10}
.tab{flex:1;text-align:center;padding:9px 4px;border:1px solid #eceae3;border-radius:10px;background:#fff;font-size:13px;font-weight:600;color:#888;cursor:pointer;user-select:none;white-space:nowrap}
.tab.active{background:#222;color:#fff;border-color:#222}
.tabpane{display:none}
.tabpane.active{display:block}
.kline-card{border-left:4px solid #9a6700}
.kimg{width:100%;height:auto;border-radius:8px;margin-top:8px;display:block}
.klegend{font-size:11px;color:#aaa;margin-top:6px}
.vbox{display:flex;justify-content:space-between;flex-wrap:wrap;gap:4px;font-size:13px;background:#f6f5f1;border-radius:8px;padding:8px 10px;margin-top:8px}
.heat-card{border-left:4px solid #c0392b}
.hrow{display:flex;align-items:center;gap:10px;padding:10px 0;border-top:1px solid #f0eee7}
.hrow:first-of-type{border-top:none}
.hrank{width:24px;height:24px;flex:none;border-radius:50%;background:#222;color:#fff;font-size:12px;font-weight:600;display:flex;align-items:center;justify-content:center}
.hmeta{font-size:12px;color:#999;margin-top:2px}
code{background:#f0eee7;padding:1px 5px;border-radius:4px;font-size:12px}
.sum-card{border-left:4px solid #1a7f37}
.sline{font-size:14px;line-height:1.65;padding:4px 0 4px 12px;margin:4px 0 12px}
.sgrid{display:grid;grid-template-columns:repeat(3,1fr);gap:8px}
.sbox{background:#f6f5f1;border-radius:8px;padding:8px 10px;text-align:center}
.beginner{font-size:12.5px;line-height:1.75;color:#555;background:#f3f7f3;border:1px solid #e4eee4;border-radius:10px;padding:11px 13px;margin-top:12px}
"""

TAB_SCRIPT = (
    "<script>function showTab(i){"
    "var t=document.querySelectorAll('.tab'),p=document.querySelectorAll('.tabpane');"
    "for(var k=0;k<t.length;k++){"
    "if(k===i){t[k].classList.add('active');}else{t[k].classList.remove('active');}}"
    "for(var k=0;k<p.length;k++){"
    "if(k===i){p[k].classList.add('active');}else{p[k].classList.remove('active');}}"
...
def build_news_summary(news, mkt_text):
    """新聞下方的白話重點統整，給股票新手看的"""
    if not news:
        return ""
    pos = sum(1 for n in news if title_polarity(n["title"]) > 0)
    neg = sum(1 for n in news if title_polarity(n["title"]) < 0)
    neu = len(news) - pos - neg

    if pos >= neg * 1.3 and pos > neu:
        mood, mcol = "偏樂觀", "#1a7f37"
    elif neg >= pos * 1.3 and neg > neu:
        mood, mcol = "偏謹慎", "#b42318"
    else:
        mood, mcol = "多空拉鋸、中性", "#9a6700"

    # 找出新聞裡最常被討論的主題
    themes = []
    for theme, kws in SECTOR_TERMS.items():
        cnt = sum(1 for n in news if any(k in n["title"] for k in kws))
        if cnt:
            themes.append((theme.replace("組合", ""), cnt))
    themes.sort(key=lambda x: x[1], reverse=True)
    top = themes[:3]
    if top:
        theme_names = "、".join(t for t, _ in top)
        theme_txt = "、".join(f"{t}（{c}則）" for t, c in top)
    else:
        theme_names = "沒有特別集中的主題"
        theme_txt = "無明顯集中話題"

    summary_line = (
        f"今天新聞氣氛{mood}，最常被討論的是 {theme_names}。"
        f"再加上「{mkt_text}」，整體可以先當成「{mood}」的背景來看。"
    )

    chips = (
        '<div class="sgrid">'
        f'<div class="sbox"><div class="zlab">利多新聞</div><div class="zval up">{pos} 則</div></div>'
        f'<div class="sbox"><div class="zlab">利空新聞</div><div class="zval down">{neg} 則</div></div>'
        f'<div class="sbox"><div class="zlab">中性</div><div class="zval flat">{neu} 則</div></div>'
        "</div>"
    )

    beginner = (
        '<div class="beginner"><b>新手看這裡 🔰</b><br>'
        "・<b>利多</b>＝對股價「可能」有利的消息（例如獲利成長、接到大單）；"
        "<b>利空</b>＝「可能」不利的消息（例如砍單、業績衰退）。<br>"
        "・新聞氣氛只是「當下的情緒」，它會變、而且常常已經反映在股價裡了，"
        "所以<b>不能只看到利多就買、看到利空就賣</b>。<br>"
        "・比較穩的看法：把新聞當成「背景天氣」，真正要不要動，還是回去看個股自己的"
        "趨勢（前面幾頁的分數、K 線、量能）。<br>"
        "・這頁全部是程式自動統計的，<b>不是投資建議</b>，參考就好、別當明牌。</div>"
    )

    return (
        '<div class="card sum-card"><div class="gname">重點統整 · 白話版</div>'
        f'<div class="sline" style="border-left:3px solid {mcol}">{esc(summary_line)}</div>'
        f"{chips}"
...
def analyze_volume(df):
    """評估前一交易日成交量 vs 近 20 日均量，並給量價關係的客觀描述"""
    v = df["Volume"].dropna()
    c = df["Close"].dropna()
    if len(v) < 21 or len(c) < 2:
        return None
    last_v = float(v.iloc[-1])
    avg20 = float(v.iloc[-21:-1].mean())  # 前一日「之前」的 20 日均量
    ratio = (last_v / avg20) if avg20 > 0 else 0.0
    chg = (c.iloc[-1] / c.iloc[-2] - 1) * 100  # 前一日漲跌幅
    ...
    up = chg >= 0
    if vtag in ("爆量", "量增"):
        verdict = "價漲量增，買盤積極" if up else "價跌量增，賣壓較重"
    elif vtag == "量縮":
        verdict = "價漲量縮，追價意願轉弱" if up else "價跌量縮，賣壓暫歇"
    else:
        verdict = "量能持平"

    return {
        "lots": round(last_v / 1000),  # 股數換算成「張」(1 張=1000 股)
        "avg_lots": round(avg20 / 1000),
        "ratio": ratio,
        "vtag": vtag,
        "chg": round(chg, 1),
        "up": up,
...
def build_kline_png(df, days=120):
    """用 mplfinance 畫 K 線(疊 5/20/60 均線)+ 量，回傳 base64 內嵌圖"""
    d = df.tail(days)[["Open", "High", "Low", "Close", "Volume"]].copy()
    buf = io.BytesIO()
    # 台股慣例：紅漲綠跌
    mc = mpf.make_marketcolors(
        up="#d23b32", down="#1a9c4f", edge="inherit",
        wick="inherit", volume="in",
    )
    style = mpf.make_mpf_style(
        marketcolors=mc, gridstyle=":", facecolor="white", figcolor="white",
        mavcolors=["#ff00aa", "#0088cc", "#ffaa00"],  # 對應 MA5/MA20/MA60
    )
    mpf.plot(
        d, type="candle", mav=(5, 20, 60), volume=True, style=style,
        figratio=(16, 9), figscale=1.0, tight_layout=True,
        savefig=dict(fname=buf, dpi=80, format="png"),
    )
    buf.seek(0)
...
def build_kline_tab_html(items):
    """items: [{name, code, df}]，每檔畫一張 K 線並附前一日量能評估"""
    if not HAS_MPF:
        return (
            '<div class="card"><div class="gname">K 棒 + 量能</div>'
            '<div class="gsum">需要 mplfinance 套件才能畫 K 線圖。請在 GitHub 的 '
            '<code>update.yml</code> 把那行 pip install 改成：<br>'
            '<code>pip install yfinance requests pandas mplfinance</code><br>'
            '存檔後重跑一次即可。</div></div>'
        )
    if not items:
        return '<div class="card"><div class="gname">K 棒 + 量能</div><div class="gsum">沒有可顯示的持股資料。</div></div>'
    cards = ""
    for it in items:
        vol = it.get("vol")
        if vol:
            vcol = "#d23b32" if vol["up"] else "#1a9c4f"
            vsign = "+" if vol["up"] else ""
            vol_line = (
                f'<div class="vbox">'
                f'<span>前一日成交量 <b>{vol["lots"]:,}</b> 張'
                f'（近20日均量 {vol["avg_lots"]:,} 張，<b>{vol["ratio"]:.1f}</b> 倍 → '
                f'<b style="color:#9a6700">{vol["vtag"]}</b>）</span>'
                f'<span style="color:{vcol};font-weight:600">當日 {vsign}{vol["chg"]}%</span>'
                f'<span style="width:100%;color:#555;margin-top:2px">{vol["verdict"]}</span>'
                f'</div>'
            )
        else:
            vol_line = '<div class="vbox"><span style="color:#999">成交量資料不足</span></div>'
        try:
            img = build_kline_png(it["df"])
            img_tag = f'<img class="kimg" src="{img}" alt="{esc(it["name"])} K線">'
        except Exception as e:
            img_tag = f'<div class="gsum">此檔 K 線繪製失敗：{esc(str(e))}</div>'
        cards += (
            f'<div class="card kline-card">'
            f'<div class="gname">{esc(it["name"])} <span style="color:#aaa;font-size:12px">{esc(it["code"])}</span></div>'
            f'{vol_line}{img_tag}'
            f'<div class="klegend">均線：'
            f'<span style="color:#f0a">MA5</span>　'
            f'<span style="color:#08c">MA20</span>　'
            f'<span style="color:#fa0">MA60</span>　·　紅K漲、綠K跌（台股慣例）</div>'
            f'</div>'
        )
    return cards
...
# ==================== [ 熱度榜 分頁 ] ====================
def build_heat_tab_html(rows):
    """rows 已排序：同時具新聞熱度 + 技術動能的個股"""
    head = (
        '<div class="card heat-card"><div class="gname">熱度榜 — 有討論、動能也強</div>'
        '<div class="gsum">從候選股池中，篩出「近期被新聞提到」且「技術動能偏強」的個股，'
        '依綜合熱度排序。<b>這不是預測、更不是買賣建議</b>，只是把客觀資料整理給你參考。</div>'
    )
    if not rows:
        return head + '<div class="gsum" style="margin-top:8px">目前沒有同時具備新聞熱度與強勢動能的個股，這很正常，盤整期常常是空的。</div></div>'
    body = ""
    for i, r in enumerate(rows, 1):
        ccls = "up" if r["chg"] > 0 else ("down" if r["chg"] < 0 else "flat")
        csign = "+" if r["chg"] > 0 else ""
        body += (
            f'<div class="hrow">'
            f'<div class="hrank">{i}</div>'
            f'<div style="flex:1;min-width:0">'
            f'<div style="font-weight:600">{esc(r["name"])} <span style="color:#aaa;font-size:12px">{esc(r["code"])}</span></div>'
            f'<div class="hmeta">趨勢分 {r["score"]}　·　5日 {("+" if r["mom5"]>=0 else "")}{r["mom5"]:.1f}%　·　新聞提及 {r["news"]} 次</div>'
            f'</div>'
            f'<div class="{ccls}" style="font-weight:600;white-space:nowrap">{csign}{r["chg"]}%</div>'
            f'</div>'
        )
...
def build_html(results, mkt_text, mkt_cls, news_html="", holdings_html="",
               kline_html="", heat_html="", news_summary_html=""):
    ...
    return f'<!DOCTYPE html><html lang="zh-Hant"><head><meta charset="utf-8"><meta http-equiv="refresh" content="900"><meta http-equiv="Cache-Control" content="no-cache, no-store, must-revalidate"><meta http-equiv="Pragma" content="no-cache"><meta http-equiv="Expires" content="0"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="apple-mobile-web-app-capable" content="yes"><meta name="mobile-web-app-capable" content="yes"><meta name="apple-mobile-web-app-status-bar-style" content="default"><meta name="apple-mobile-web-app-title" content="台股趨勢"><meta name="theme-color" content="#f6f5f1"><link rel="apple-touch-icon" href="icon.png"><title>台股每日趨勢</title><style>{CSS}</style></head><body><div class="wrap"><h1>台股組合 · 每日趨勢</h1><div class="sub">更新時間：{now}　·　<span style="color:{mkt_color};font-weight:600">{esc(mkt_text)}</span></div><div class="tabs"><div class="tab active" onclick="showTab(0)">我的持股</div><div class="tab" onclick="showTab(1)">新聞</div><div class="tab" onclick="showTab(2)">K棒量能</div><div class="tab" onclick="showTab(3)">熱度榜</div></div><div class="tabpane active"><div class="note">「消息面」是用新聞關鍵字粗略判斷（會漏判或讀錯，僅供參考）。「一致」＝消息面與趨勢同向；「背離」＝兩者相反，要提高警覺，請以趨勢面為主。</div>{holdings_html}{cards}<div class="legend"><span class="up">●</span> 偏多/上漲　<span class="flat">●</span> 觀望/盤整　<span class="down">●</span> 偏空/下跌</div></div><div class="tabpane">{news_html}{news_summary_html}</div><div class="tabpane"><div class="note">你持股的日 K 線（疊 5/20/60 日均線）與成交量，並評估前一交易日量能。</div>{kline_html}</div><div class="tabpane">{heat_html}</div><div class="foot">股價來源：Yahoo Finance；新聞來源：Google 新聞（個人用途）。本表為自動計算之參考訊號，非投資建議；過去走勢不代表未來，投資決策與風險請自行評估。</div></div>{TAB_SCRIPT}</body></html>'
...
def main():
    ...
    news_summary_html = build_news_summary(news_pool, mkt_text)
    ...
    for _, row in df_cloud.iterrows():
        ...
        dfk = fetch_df(code)  # 完整 OHLCV（已快取，不會重抓）
        if dfk is None:
            continue
        c = dfk["Close"].dropna()
        ...
        # K 棒分頁用：附上完整資料與前一日量能評估
        kline_items.append(
            {
                "name": names_dict.get(code, code),
                "code": code,
                "df": dfk,
                "vol": analyze_volume(dfk),
            }
        )
    holdings_html = build_holdings_html(hold_items)
    kline_html = build_kline_tab_html(kline_items)

    # 熱度榜：候選股池中，找「有新聞熱度」且「技術動能也強」的
    print("開始計算熱度榜…")
    heat_rows = []
    for code, name in HEAT_CANDIDATES.items():
        news_hits = sum(1 for n in news_pool if name in n["title"])
        if news_hits < 1:  # 沒被新聞提到就跳過（要有討論熱度）
            continue
        c = fetch(code)
        if c is None:
            continue
        sc, chg1, mom5 = analyze(c)
        if sc < 55 and mom5 < 2:  # 動能也要夠強才入榜
            continue
        heat = news_hits * 12 + (sc - 50) + mom5  # 綜合熱度（新聞權重高一點）
        heat_rows.append(
            {
                "name": name, "code": code, "score": sc,
                "chg": chg1, "mom5": mom5, "news": news_hits, "heat": heat,
            }
        )
    heat_rows.sort(key=lambda x: x["heat"], reverse=True)
    heat_rows = heat_rows[:12]
    print(f"  熱度榜：{len(heat_rows)} 檔入榜")
    heat_html = build_heat_tab_html(heat_rows)

    html = build_html(
        results, mkt_text, mkt_cls, news_html, holdings_html,
        kline_html, heat_html, news_summary_html,
    )
